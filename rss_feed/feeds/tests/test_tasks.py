...
class TestTasks(TestCase):
    ...
    def setUp(self):
        self.feed = FeedFactory(is_followed=True, is_auto_updated=True)
        ItemFactory.create_batch(4, feed=self.feed)
        self.not_followed_feed = FeedFactory(is_followed=False, is_auto_updated=True)
        ItemFactory.create_batch(3, feed=self.not_followed_feed)
        self.not_auto_updated_feed = FeedFactory(is_followed=True, is_auto_updated=False)
